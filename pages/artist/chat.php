...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    requireCsrf('/smartprint-workflow/pages/artist/chat.php');
    ...
    if ($action !== 'send_message') {
        ...
        header('Location: /smartprint-workflow/pages/artist/chat.php');
        exit;
    }

    if ($jobId <= 0 || $message === '') {
        setFlash('error', 'Select a job and enter a message.');
        header('Location: /smartprint-workflow/pages/artist/chat.php?job_id=' . $jobId);
        exit;
    }
    ...
    $accessStmt = $pdo->prepare("
        SELECT id
        FROM print_jobs
        WHERE id = ? AND assigned_artist_id = ? AND client_id IS NOT NULL
        LIMIT 1
    ");
    $accessStmt->execute([$jobId, $user['id']]);

    if (!$accessStmt->fetch()) {
        setFlash('error', 'You can only chat on jobs assigned to you.');
        header('Location: /smartprint-workflow/pages/artist/chat.php');
        exit;
    }
...
$jobsStmt = $pdo->prepare("
    SELECT j.id, j.reference_number, j.title, j.updated_at, c.full_name AS client_name
    FROM print_jobs j
...
$pageTitle = 'Client Chat';
require_once __DIR__ . '/../../includes/header.php';
?>

<section class="card">
    <h2>Chat With Assigned Clients</h2>
    <p>Only your assigned jobs are available for messaging.</p>
    <?php if ($error): ?><div class="alert error"><?= e($error) ?></div><?php endif; ?>
    <?php if ($success): ?><div class="alert success"><?= e($success) ?></div><?php endif; ?>

    <div class="chat-layout">
        <aside class="chat-job-list">
            <h3>Assigned Threads</h3>
            <?php if (!$jobs): ?>
                <p class="assignment-empty">No assigned job threads yet.</p>
            <?php endif; ?>
            <?php foreach ($jobs as $job): ?>
                <a class="chat-job-link <?= (int) $job['id'] === $selectedJobId ? 'active' : '' ?>" href="/smartprint-workflow/pages/artist/chat.php?job_id=<?= (int) $job['id'] ?>">
                    <strong><?= e($job['reference_number']) ?></strong>
                    <span><?= e($job['title']) ?></span>
                    <small>Client: <?= e($job['client_name']) ?></small>
                </a>
            <?php endforeach; ?>
        </aside>
        ...
        <div class="chat-thread-wrap">
            ...
            <?php else: ?>
                <div class="chat-thread-head">
                    ...
                    <p>Client: <?= e($selectedJob['client_name']) ?></p>
                </div>
